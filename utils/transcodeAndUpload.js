...
const transcodeAndUpload = async (videoFilePath) => {
     try {
          const transcodedFilePath = path.join(
               ...
               'public',
               'temp',
               `videoFile-${Date.now()}-${Math.floor(Math.random() * 1000000000)}.mp4_transcoded_${Date.now()}.mp4`
          );

          const ffmpegCommand = `C:\\Users\\saura\\Desktop\\youtube\\backend\\node_modules\\@ffmpeg-installer\\win32-x64\\ffmpeg.exe -i ${videoFilePath} -vf scale=1280:-1 -c:v libx264 -crf 23 -preset medium -c:a aac -b:a 128k ${transcodedFilePath}`;
